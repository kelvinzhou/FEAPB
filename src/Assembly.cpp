...
void Assembly(FEAPB::ReadData &InputData, Eigen::MatrixXi &conc, Eigen::MatrixXd &nodx, double &h, int &C,
              int &n, Eigen::SparseMatrix<double> &stiffg, Eigen::SparseMatrix<double> &massg,
              Eigen::VectorXd &F, Eigen::MatrixXd &Dirichlet_data){
    ...
    FEAPB::PiezoAreaMomentsTensors(InputData.IsoElast, InputData.densi, InputData.Elast, InputData.Piezo,
                                   InputData.Die, InputData.shfac, InputData.x1, InputData.x2, InputData.NInt,
                                   InputData.AIso, InputData.Analysis,H,HD);
    ...
    for (int ielem = 0; ielem < n; ielem++)
    {
        ...
        // RHS Assembly
        element.RHS(rhsel, Nmat_force, conc, nodx, zw, InputData.pForce, InputData.pForcePos,
                    InputData.udl_Force, C, h, ielem, BasesFunc);
