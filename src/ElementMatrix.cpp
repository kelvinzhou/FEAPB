...
std::tuple<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>, Eigen::Matrix<double, Eigen::Dynamic,
Eigen::Dynamic>, Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>>
ElementMatrix::Submatrices(Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>& zwbases,
                           Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic>& zwgbases, double h, int _C)
{
...
//        std::cout << Nmat.block(0,0,6,6) << std::endl << std::endl;
    ...
    submatrices << B_all.rows(), B_all.cols(), Nmat_all.rows(), Nmat_all.cols(),
            Nmat_force_all.rows(), Nmat_force_all.cols();
...
void ElementMatrix::Stiffness(Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>& stiffel,
                              Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic>& B, Eigen::Matrix<double,
                              Eigen::Dynamic,Eigen::Dynamic>& zw, Eigen::Matrix<double,15,15>& H, double &L)
{
...
void ElementMatrix::Mass(Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>& massel, Eigen::Matrix<double,
                         Eigen::Dynamic,Eigen::Dynamic>& Nmat, Eigen::Matrix<double, Eigen::Dynamic,
                         Eigen::Dynamic> &zw, Eigen::Matrix<double,6,6> &HD, double &L){
...
// Elemental RHS
void ElementMatrix::RHS(Eigen::Matrix<double,Eigen::Dynamic,1>& rhsel,
                        Eigen::Matrix<double,Eigen::Dynamic, Eigen::Dynamic>& Nmat_force, Eigen::MatrixXi &conc,
                        Eigen::MatrixXd &nodx, Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> &zw,
                        Eigen::MatrixXd &P, Eigen::MatrixXd &Pl, Eigen::MatrixXd &UDL, int _C, double &L,
                        int &ielem,std::string &BasesFunc)
{
    // Compute UDLs
    ...
    for (int i=0; i<UDL.rows(); i++)
    {
        ...
        if (ielem>=UDL(i,2)-1 && ielem<=(UDL(i,2)+UDL(i,3)-2))
        {
            ...
            else if (UDL(i,1)==3 || UDL(i,1)==6 || UDL(i,1)==9)
            {throw std::invalid_argument("UDL cannot be applied in the cross sectional direction (3,6,9)");}
    ...
    for (int i=0; i<P.rows(); i++)
    {
        ...
        if (ielem==conc.rows()-1){
            dist_next_elem = (_C+1)*nodx.row(conc(ielem+1,0)-1).norm()+nodx.row(conc(ielem,0)-1).norm();
        }
